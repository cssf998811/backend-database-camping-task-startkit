...
UPDATE "COACH" SET experience_years = 3 WHERE user_id =
(SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io');

UPDATE "COACH" SET experience_years = 5 WHERE  user_id =
...
SELECT 
  u.name,
  u.id,
  COALESCE(cp.total_credits, 0) - COALESCE(cb.total_courses, 0) AS remaining_credit
FROM 
  "USER" u
INNER JOIN (
  SELECT 
...
where
  "COACH_LINK_SKILL".skill_id = (select id from "SKILL" where name = '重訓')
ORDER BY
